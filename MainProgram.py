...
class MainWindow(QMainWindow):
    ...
    def signalconnect(self):
        ...
        self.ui.ExitBtn.clicked.connect(self.quit_project)
        self.ui.FilesBtn.clicked.connect(self.detact_batch_imgs)
        self.ui.WightBtn.clicked.connect(self.wight_change)
        self.ui.stopBtn.clicked.connect(self.stop_short_time)
    ...
    def open_img(self):
        ...
        #   默认显示第一个目标框坐标
        #   设置坐标位置值
            # self.ui.label_xmin.setText("111")
    ...
    def wight_change(self):

        #wight_path, _ = QFileDialog.getOpenFileName(self.ui.centralwidget, '选择权重', './', "wight files (*.pt)")
        wight_path, _ = QFileDialog.getOpenFileName(None, '选择权重', './', "wight files (*.pt)")
        if not wight_path:
            return

        # 加载检测模型
        self.ui.WightlineEdit.setText(wight_path)
        self.model = YOLO(wight_path, task='detect')
        self.model(np.zeros((48, 48, 3)))  # 预先加载推理模型
        self.fontC = ImageFont.truetype("Font/platech.ttf", 25, 0)
    ...
    def quit_project(self,event):
        # 创建一个QMessageBox实例
        reply = QMessageBox.question(self, '退出确认',
                                     '请确定你保存后选择退出', QMessageBox.Yes |
                                     QMessageBox.No)
        # 检查用户的选择
        if reply == QMessageBox.Yes:
            QCoreApplication.quit()  # 接受关闭事件
